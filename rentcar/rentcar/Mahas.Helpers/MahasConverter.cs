...
public class MahasConverter
{
    private readonly static List<string> ImagesExtentions = new List<string>()
    {
        "jpg",
        "jpeg",
        "jfif",
    ...
    public static bool IsImage(string extension)
    {
        ...
        if (extension.IndexOf(".") > 0 && !extension.EndsWith("."))
        {
            extension = extension.Split('.').Last();
        }
        return ImagesExtentions.Any(x => x == extension);
    }
    ...
    public static void Cast<T, U>(T modelFrom, U modelTo, params string[] exclude)
    {
        if (modelFrom == null) return;
        if (modelTo == null) throw new Exception("modelTo is null");

        var properties = modelFrom.GetType().GetProperties();
        var modelToType = modelTo.GetType();

        foreach (var property in properties)
        {
            if (exclude.FirstOrDefault(x => x == property.Name) == null)
            {
                modelToType.GetProperty(property.Name)?.SetValue(modelTo, property.GetValue(modelFrom));
            }
        }
    }
    ...
    public static string GetMimeType(string extension)
    {
        ...
        string? mime;
        Mappings.TryGetValue(extension, out mime);
