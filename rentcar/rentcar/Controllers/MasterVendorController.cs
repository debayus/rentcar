...
public class MasterVendorController : Controller
{
    ...
    private readonly ILogger<MasterVendorController> _logger;
    private readonly IUserStore<IdentityUser> _userStore;
    private readonly IUserEmailStore<IdentityUser> _emailStore;
    private readonly UserManager<IdentityUser> _userManager;

    private string roleName = "Vendor";

    public MasterVendorController(
        ApplicationDbContext db,
        ILogger<MasterVendorController> logger,
    ...
    private new IEnumerable<MasterVendorViewModel> ViewData
    {
        get
        {
            return _db.mVendor.Select(x => new MasterVendorViewModel
            {
                Id = x.Id,
                Nama = x.Nama,
    ...
    {
        IEnumerable<MasterVendorViewModel> query = ViewData;
            ...
            switch (_filter.Key)
            {
                case "Filter":
                    ...
                    query = query.Where(x =>
                        ...
                        (x.Alamat ?? "").ToUpper().Contains(_filter.Value) ||
        ...
        Func<MasterVendorViewModel, dynamic> _orderBy = (x =>
            orderBy == 0 ? x.Id :
            orderBy == 1 ? x.Nama :
    ...
    [ValidateAntiForgeryToken]
    [ActionName("Data")]
    public async Task<IActionResult> Post(MasterVendorPostViewModel model)
    {
        ...
        var hasName = _db.mVendor.Where(x => x.Nama.ToUpper() == model.Nama.ToUpper()).Any();
        if (hasName)
        {
        ...
        if (role == null)
        {
            ModelState.AddModelError("Name", "Vendor role is not exsit");
            return BadRequest(ModelState);
        }
        ...
        if (result.Succeeded)
        {
            ...
            _db.mVendor.Add(new VendorDbModel()
            {
                Nama = model.Nama,
                Id_User = user.Id,
            });
    ...
    [ValidateAntiForgeryToken]
    [ActionName("Data")]
    public IActionResult Put(int Id, MasterVendorPutViewModel model)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
        ...
        model.Nama = model.Nama.Trim();
        var hasName = _db.mVendor.Where(x => x.Nama.ToUpper() == model.Nama.ToUpper() && x.Id != Id).Any();
        if (hasName)
        {
            ModelState.AddModelError("Name", "Name already exists");
            return BadRequest(ModelState);
        ...
        var data = _db.mVendor.FirstOrDefault(x => x.Id == Id);
